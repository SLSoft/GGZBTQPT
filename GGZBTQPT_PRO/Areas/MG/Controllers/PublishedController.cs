...
namespace GGZBTQPT_PRO.Areas.MG.Controllers
{
    ...
    public class PublishedController : BaseController
    {
        ...
        public ActionResult PublishedFinancials(int member_id, int id = 1)
        {
            ...
            try
            {
                PagedList<T_XM_Financing> finacials = db.T_XM_Financing.Where(p => p.IsValid == true).OrderByDescending(f => f.CreateTime).Where(f => f.MemberID == member.ID && f.IsValid == true).ToPagedList(id, 5);
        ...
        public ActionResult PublishedInvestments(int member_id, int id = 1)
        {
            ...
            try
            {
                PagedList<T_XM_Investment> investments = db.T_XM_Investment.Where(p => p.IsValid == true).OrderByDescending(f => f.CreateTime).Where(f => f.MemberID == member.ID && f.IsValid == true).ToPagedList(id, 5);
        ...
        public ActionResult PublishedProducts(int member_id, int id = 1 )
        {
            ...
            try
            {
                PagedList<T_JG_Product> products = db.T_JG_Product.Where(p => p.IsValid == true).OrderByDescending(p => p.CreateTime).Where(p => p.MemberID == member.ID && p.IsValid == true).ToPagedList(id, 5);
